        ...
        ScrollView {
            HStack {
                VStack(alignment: .leading) {
                    ...
                    Text("When you first opened Multiqueue, you were presented with two alerts. Each one asked you to grant a permission to Multiqueue. Regardless of your initial choice, these alerts will never appear again; to modify your permission choices, visit Settings > Privacy. From there, choose permission you would like to edit and find Multiqueue in the list.")
                        .fixedSize(horizontal: false, vertical: true)
                    
                    Text("Why Multiqueue Needs Permissions")
                        .fontWeight(.bold)
                        .multilineTextAlignment(.leading)
                        .fixedSize(horizontal: false, vertical: true)
                        .padding(.top)
                    
                    Text("The main function of Multiqueue is to allow multiple devices to contribute to a single music queue on a single device. In order to accomplish this, Multiqueue firstly opens a local connection between the participant devices. Then, it allows these devices to look up songs from the Apple Music catalog. Finally, it facilitates the transfer of these songs between the devices. These tasks require use of your device's music information (to look up songs) and its connection capabilities (to interface with nearby devices). Thus, your authorization to use the Media & Apple Music and Local Network permissions are essential for Multiqueue to function.")
                        .fixedSize(horizontal: false, vertical: true)
                    ...
                    Text("The world of privacy and permissions can be confusing, and it can be hard to tell which permissions are essential for an app and which are not. Since both permissions Multiqueue requests are essential, you'll see messages in red boxes on the main menu if either or both permissions are revoked. These messages inform you what permissions should be enabled and why Multiqueue needs the permissions.")
                        .fixedSize(horizontal: false, vertical: true)
                    ...
                    Text("Multiqueue only needs its permissions when you're actively using the app and in a queue-sharing session. So if you perfer to disable Multiqueue's permissions when not using the app, and re-enable them when you are, please know that this will not affect your app experince in any way. Only note that if permissions are revoked while in a queue-sharing session, you may be kicked back to the main menu and the session may be closed abruptly.")
                        .fixedSize(horizontal: false, vertical: true)
